...
cd deteccion-ventanas-streamlit
```
...
Confirma que `models/` contenga los `.pt` / `.pth` descargados vía Git LFS y no punteros vacíos:
...
├── .gitattributes            # Reglas de Git LFS para *.pt y *.pth
└── README.md
```
